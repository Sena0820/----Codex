...
- workflow は Node 24 対応の公式 Actions バージョンを使っています。
...
- GitHub Pages は公開URLを知っていれば誰でも見られるため、認証付きの非公開サイトにはできません。代わりに `noindex` と `robots.txt` で検索流入は抑えています。
- ブックマークは同じブラウザ内の `localStorage` に保存されるため、同じスマホ・同じブラウザなら次回訪問時にも残ります。
